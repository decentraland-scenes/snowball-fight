...
export class Ball {
    ...
    constructor(room:Room, _teamColor:teamColor){
        ...
        this.ballEntity.addComponent(new Transform({position: new Vector3(8,8,8),
        ...
        this.bounceEmitter.addComponent(new Transform({position: new Vector3(0,0,0)}))
        ...
        if(this.active){
           // this.hitSound.playOnce()
            let explosion = new Explosion(
                _hitPoint, 
                Quaternion.FromToRotation(Vector3.Up(), _normal.rotate(Quaternion.RotationAxis(_normal,Math.random()*360))
                ))
            this.active = false
            this.hide()            
        }
           ...
           // worldNoGravity.step(FIXED_TIME_STEPS, dt, MAX_TIME_STEPS)
            ...
            for(let ball of this.ballManager.balls){
                ...
                if(ball.active) {   
                    ...
                    let originPos = new Vector3(transform.position.x, transform.position.y, transform.position.z)
                    let targetPos = new Vector3(nextPos.x, nextPos.x, nextPos.x)
                    
                    let moveVector = transform.position.subtract(nextPos).multiplyByFloats(1,0,1)
                    ...
                    //move ball
                    if(ball.currentSpeed  < 0.05 && transform.position.y < 2*ball.ballRadius + 0.1){
                        ball.currentSpeed = 0
                        ...
                        let rotateAxis = Vector3.Cross(ball.moveVector, Vector3.Down()).normalize()
