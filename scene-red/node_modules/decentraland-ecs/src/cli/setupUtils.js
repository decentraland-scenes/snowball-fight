...
var getFilesFromFolder = function (_a) {
    var folder = _a.folder, addOriginalPath = _a.addOriginalPath, ignorePattern = _a.ignorePattern, customHashMaker = _a.customHashMaker, rootFolder = _a.rootFolder;
    ...
    return filteredFiles
        ...
        var absoluteFolder = rootFolder ? rootFolder.replace(/\\/gi, '/') : folder.replace(/\\/gi, '/');
        var relativeFilePathToRootFolder = absolutePath.replace(absoluteFolder, '').replace(/^\/+/, '');
        var relativeFilePathToFolder = file.replace(absoluteFolder, '').replace(/^\/+/, '');
        return {
...
function entityV3FromFolder(_a) {
    var folder = _a.folder, addOriginalPath = _a.addOriginalPath, ignorePattern = _a.ignorePattern, customHashMaker = _a.customHashMaker, catalystRootFolder = _a.catalystRootFolder;
    ...
    if (fs.existsSync(assetJsonPath)) {
        try {
            ...
            if (schemas_1.sdk.AssetJson.validate(assetJson) && assetJson.assetType === schemas_1.sdk.ProjectType.PORTABLE_EXPERIENCE) {
                isParcelScene = false;
            }
        }
        catch (err) {
...
function getSceneJson(_a) {
    var baseFolders = _a.baseFolders, pointers = _a.pointers, customHashMaker = _a.customHashMaker, catalystRootFolder = _a.catalystRootFolder;
    ...
    var allDeployments = baseFolders.map(function (folder) {
        ...
        return entityV3FromFolder({
            ...
            customHashMaker: customHashMaker,
            catalystRootFolder: catalystRootFolder
        });
    });
